...
// Cloudflare Pages Function for /api/tasks/process
// Handles AI processing of task lists into organized stories
...
function preprocessTask(task: string): PreprocessedTask {
  ...
  const processedText = task.replace(frogPattern, '').replace(/\s+/g, ' ').trim();
...
function analyzeTaskText(task: string): TaskAnalysis {
  const timePatterns = [/(\d+)\s*(?:hour|hr|h)s?/i, /(\d+)\s*(?:minute|min|m)s?/i, /(\d+):(\d+)/];
  ...
  const category = Object.entries(categoryKeywords).find(([_, pattern]) =>
    pattern.test(task)
  )?.[0] as ProjectCategory | undefined;
...
const ALLOWED_TASK_CATEGORIES: Exclude<TaskCategory, 'break'>[] = [
  'focus',
  'learning',
  'review',
  'research',
      ...
      try {
        ...
        processedData = {
          stories: (parsedData.stories || []).map((story: any) => ({
            ...story,
            tasks: Array.isArray(story.tasks) ? story.tasks.map(transformTaskData) : [],
          })),
        };
        ...
        processedData.stories.forEach((story: { tasks: { id?: string }[] }) => {
          story.tasks.forEach(
            (task: {
              ...
              if (task.duration && task.duration >= 60 && task.suggestedBreaks.length === 0) {
              ...
              if (task.duration && task.duration % 5 !== 0) {
                task.suggestedBreaks.push({
                  after: 0,
                  duration: 0,
