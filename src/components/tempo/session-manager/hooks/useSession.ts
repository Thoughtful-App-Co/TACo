...
export const useSession = ({
  ...
  // Timer state
  const [activeTimeBox, setActiveTimeBox] = createSignal<{ storyId: string; timeBoxIndex: number } | null>(null)
  ...
  // Handle task click
  const handleTaskClick = (storyId: string | undefined, timeBoxIndex: number, taskIndex: number, task: TimeBoxTask) => {
    if (!session() || !storyId) {
