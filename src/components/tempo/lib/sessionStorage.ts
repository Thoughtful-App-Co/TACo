...
export const sessionStorage = {
      ...
      const updatedStoryBlocks = sessionData.storyBlocks.map((story: StoryBlock) => {
        if (story.id === storyId && story.timeBoxes[timeBoxIndex]) {
          ...
          if (status === 'completed' && updatedTimeBoxes[timeBoxIndex].tasks) {
            updatedTimeBoxes[timeBoxIndex].tasks = updatedTimeBoxes[timeBoxIndex].tasks.map(
              (task: TimeBoxTask) => ({
                ...task,
      ...
      if (story.id === storyId && story.timeBoxes[timeBoxIndex]) {
        ...
        if (timeBox.tasks && timeBox.tasks[taskIndex]) {
          ...
          const allTasksCompleted = updatedTasks.every(
            (task: TimeBoxTask) => task.status === 'completed'
          );
          const anyTaskCompleted = updatedTasks.some(
            (task: TimeBoxTask) => task.status === 'completed'
          );
          const timeBoxStatus: BaseStatus = allTasksCompleted
            ? 'completed'
            : anyTaskCompleted
              ? 'in-progress'
              : 'todo';
  ...
  async updateSessionProgress(date: string): Promise<boolean> {
    ...
    const updatedStoryBlocks = session.storyBlocks.map((story: StoryBlock) => {
      ...
      const completedWorkBoxes = story.timeBoxes.filter(
        (box: TimeBox) => box.type === 'work' && box.status === 'completed'
      ).length;
      const progress =
        totalWorkBoxes > 0 ? Math.round((completedWorkBoxes / totalWorkBoxes) * 100) : 0;
    ...
    const allCompleted = updatedStoryBlocks.every((story: StoryBlock) =>
      story.timeBoxes
        ...
        .every((box: TimeBox) => box.status === 'completed')
    );
  ...
  normalizeSession(session: StoredSession): StoredSession {
    ...
    const validateTimeBoxType = (type?: string): TimeBoxType => {
      ...
      return type && validTypes.includes(type as TimeBoxType) ? (type as TimeBoxType) : 'work';
    };
    ...
    const normalizedStoryBlocks = storyBlocks.map((block) => {
      ...
      if (progress === undefined || progress === null) {
        ...
        const completedWorkBoxes = normalizedTimeBoxes.filter(
          (box) => box.type === 'work' && box.status === 'completed'
        ).length;
