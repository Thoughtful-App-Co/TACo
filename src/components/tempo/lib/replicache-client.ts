...
// @ts-nocheck - Replicache module not yet installed, this file is placeholder for future local-first sync
...
export function createReplicacheClient(userId: string): Rep {
  const rep = new Replicache({
    ...
    mutators: {
      async saveSession(
        tx: WriteTransaction,
        { date, session }: { date: string; session: StoredSession }
      ) {
