...
function calculateSplitDurations(totalDuration: number): {
  workDurations: number[];
  totalWithBreaks: number;
} {
...
async function analyzeAndGroupTasks(
  tasks: Task[]
): Promise<{ title: string; tasks: Task[]; suggestedDuration: number }[]> {
...
export async function createTimeBoxes(tasks: Task[]): Promise<SessionPlan> {
  ...
  for (const group of groups) {
    ...
    for (const task of group.tasks) {
      if (task.isFrog) {
        ...
        if (task.needsSplitting) {
          ...
          const splitResult = calculateSplitDurations(
            frogTracker.originalDurations.get(task.id) || 0
          );
      ...
      if (task.needsSplitting) {
        ...
        splitResult.workDurations.forEach((duration, index) => {
          ...
          if (partNumber < totalParts) {
            const breakDuration =
              workDuration >= DURATION_RULES.MAX_WORK_WITHOUT_BREAK * 0.75
            ...
            timeBoxes.push({
              ...
              type:
                workDuration >= DURATION_RULES.MAX_WORK_WITHOUT_BREAK * 0.75
                  ? ('long-break' as TimeBoxType)
                  : ('short-break' as TimeBoxType),
              icon: '⏸️',
            });
    ...
    if (timeBoxTotal !== storyDuration) {
      throw new Error(
        `Story duration mismatch for "${group.title}": ` +
  ...
  if (frogTracker.scheduled < frogTracker.total) {
    throw new Error(
      `Not all frog tasks were scheduled: ${frogTracker.scheduled}/${frogTracker.total} completed`
    );
  }
  ...
  return {
    ...
    frogMetrics: {
      ...
      scheduledWithinTarget: storyBlocks.reduce(
        (acc, block) =>
          acc +
          block.timeBoxes.filter(
            (box) =>
              box.type === 'work' && box.tasks?.some((t) => t.isFrog) && acc <= frogDeadlineMinutes
          ).length,
        0
      ),
    },
...
export function organizeTasks(tasks: Task[]): Task[] {
  return tasks.sort((a, b) => {
    ...
    if (aDiffValue >= DIFFICULTY_WEIGHTS.PHYSICAL && bDiffValue < DIFFICULTY_WEIGHTS.PHYSICAL)
      return -1;
    if (bDiffValue >= DIFFICULTY_WEIGHTS.PHYSICAL && aDiffValue < DIFFICULTY_WEIGHTS.PHYSICAL)
      return 1;
