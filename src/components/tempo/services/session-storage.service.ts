...
export class SessionStorageService {
  ...
  async archiveSession(date: string): Promise<boolean> {
    try {
      ...
      console.log(`[SessionStorageService] Successfully archived session for date: ${formattedDate}`);
      return true;
    } catch (error) {
  ...
  async unarchiveSession(date: string): Promise<boolean> {
    try {
      ...
      await this.saveSession(formattedDate, updatedSession);
      console.log(`[SessionStorageService] Successfully unarchived session for date: ${formattedDate}`);
      return true;
    } catch (error) {
