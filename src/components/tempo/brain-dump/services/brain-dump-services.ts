...
const processTasks = async (taskList: string[]) => {
  ...
  // Check content type to handle empty responses
  ...
  // If response is empty, check if the request was successful
  if (!responseText) {
    if (!response.ok) {
      throw new Error(
        `API request failed with status ${response.status}. No response body provided. This may indicate a configuration issue with your API endpoint.`
      );
    }
    throw new Error(
      'API returned an empty response. Please check your API configuration and ensure an API key is properly set.'
    );
  }
  ...
  if (contentType && contentType.includes('application/json')) {
    try {
      ...
      throw new Error(
        `Failed to parse API response as JSON: ${e instanceof Error ? e.message : 'Unknown error'}. Response: ${responseText.substring(0, 200)}`
      );
    }
  } else if (!contentType) {
    // Content-Type is null/empty, still try to parse as JSON
    try {
      data = JSON.parse(responseText);
    } catch (e) {
      throw new Error(
        `API response has no Content-Type header and is not valid JSON. Response: ${responseText.substring(0, 200)}`
      );
    }
  } else {
    throw new Error(
      `Invalid response type: ${contentType}. Expected JSON. Response: ${responseText.substring(0, 200)}`
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error((data as any)?.error || `Failed to process tasks (HTTP ${response.status})`);
  }
  ...
  if (Array.isArray(apiResponse.stories)) {
    apiResponse.stories = apiResponse.stories.map((story: any) => ({
      ...story,
...
const modifyStoriesForRetry = (stories: ProcessedStory[], error: unknown): ProcessedStory[] => {
