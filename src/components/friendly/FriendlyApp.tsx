...
const Icons = {
  ...
  Party: () => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <path d="M22 2l-4.4 4.4"></path>
      <path d="M13.8 4.2l4 4"></path>
      <path d="M19.8 10.2l-4 4"></path>
    </svg>
  ),
  Eye: () => (
...
const ToggleSwitch: Component<{ checked: boolean; onChange: (v: boolean) => void; label: string; description?: string }> = (toggleProps) => (
  <div style={{
    display: 'flex',
    ...
    <div>
      <div style={{ color: 'white', 'font-weight': m.fontWeight.medium, 'font-size': m.fontSize.sm }}>
        {toggleProps.label}
      </div>
...
const TimeGrid: Component<{
  ...
  const getBookingForSlot = (day: string, period: string): Booking | undefined => {
    ...
    return props.bookings.find(
      b => b.slot.dayOfWeek === day && 
           b.slot.period === period && 
           b.status !== 'cancelled'
    );
  };
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <span style={{ 'font-size': m.fontSize.sm, color: t.colors.textMuted }}>
          This Week
        </span>
        ...
        <For each={days}>
          {day => (
            <div style={{ flex: 1, display: 'flex', 'flex-direction': 'column', gap: '6px' }}>
              ...
              <For each={periods}>
                {period => {
                  const isAvailable = () => props.availability.some(
                    s => s.dayOfWeek === day && s.period === period
                  );
                  const isHighlighted = () => props.highlightSlots?.some(
                    s => s.dayOfWeek === day && s.period === period
                  );
                  ...
                  const getSlotStyle = () => {
                    if (isBooked()) {
                      return {
                        ...
                        'box-shadow': isOpenToJoin() ? bookingColors.openToJoinGlow : bookingColors.bookedGlow,
                      };
                    }
                    ...
                    <button
                      onClick={() => {
                        if (props.onSlotClick) {
                          props.onSlotClick({ dayOfWeek: day as any, period: period as any }, booking());
      ...
      <div style={{
        ...
        <Show when={props.showBookings}>
          ...
          <div style={{ display: 'flex', 'align-items': 'center', gap: '6px' }}>
            ...
            <span style={{ 'font-size': m.fontSize.xs, color: t.colors.textMuted }}>Open to join</span>
          </div>
        </Show>
        <div style={{ display: 'flex', 'align-items': 'center', gap: '6px' }}>
          <div style={{
...
const PublicView: Component = () => {
  ...
  const joinableBookings = createMemo(() => 
    store.state.bookings.filter(b => 
      b.isPublic && b.openToGroup && b.status === 'confirmed'
    )
  );
    ...
    if (selectedSlot() && name()) {
      if (joinMode() && selectedBooking()) {
        ...
        store.submitRequest(
          ...
          note(), 
          undefined, 
          { openToGroup: openToGroup(), isPublic: isPublic() }
        );
      }
      setSubmitted(true);
    }
  ...
  const handleSlotClick = (slot: TimeSlot, booking?: Booking) => {
    if (booking && booking.isPublic && booking.openToGroup) {
      ...
      setJoinMode(true);
    } else if (store.state.profile.availability.some(
          ...
          <Show when={joinableBookings().length > 0}>
            <GlassCard style={{ padding: '24px', 'margin-bottom': '20px' }}>
              <div style={{ 
                ...
                <div style={{ color: bookingColors.openToJoin }}><Icons.UserPlus /></div>
                <h2 style={{ 
                  margin: 0, 
                  'font-size': m.fontSize.sm, 
                  ...
                  {booking => (
                    <button
                      ...
                      style={{
                        ...
                        gap: '12px',
                        padding: '14px',
                        background: selectedBooking()?.id === booking.id 
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ 
                          ...
                          <Show when={booking.showParticipantNames && booking.participants.length > 0}>
            ...
            <GlassCard style={{ padding: '24px' }}>
              ...
              <Show when={joinMode() && selectedBooking()}>
                <div style={{
                  ...
                  <Show when={selectedBooking()!.showParticipantNames && selectedBooking()!.participants.length > 0}>
                    <div style={{ 'font-size': m.fontSize.sm, color: t.colors.textMuted }}>
                      with {selectedBooking()!.participants.map(p => p.name).join(', ')}
                    </div>
                  </Show>
                </div>
              </Show>
...
const DashboardView: Component = () => {
  const [tab, setTab] = createSignal<'schedule' | 'requests' | 'circles' | 'events' | 'settings'>('schedule');
    ...
    <div style={{
      ...
      <header style={{ padding: '24px 24px 0' }}>
        <div style={{ 
          ...
          <GradientButton
            onClick={() => store.setViewMode('public')}
            variant="ghost"
            size="sm"
          >
        ...
        <div style={{ 
          ...
          {[
            { label: 'Hosted', value: store.state.profile.stats.hangoutsHosted, color: t.colors.primary },
            { label: 'Attended', value: store.state.profile.stats.hangoutsAttended, color: t.colors.secondary },
            { label: 'Friends', value: store.state.profile.stats.friendsMade, color: t.colors.accent },
            { label: 'Streak', value: `${store.state.profile.stats.streakWeeks}w`, color: '#F59E0B' },
          ].map(stat => (
                    ...
                    <div style={{
                      ...
                      <div style={{ flex: 1, 'min-width': 0 }}>
                        ...
                        <div style={{ 
                          ...
                          <span>{booking.slot.dayOfWeek} {booking.slot.period}</span>
      ...
      <nav style={{
        ...
        {[
          ...
          { id: 'requests', icon: Icons.Inbox, label: 'Requests', badge: pendingRequests().length },
        ...
        <div style={{ 'margin-bottom': '20px' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              type="text"
              value={username()}
              onInput={(e) => setUsername(e.currentTarget.value.toLowerCase().replace(/[^a-z0-9_-]/g, ''))}
          ...
          <For each={store.state.friends}>
            {friend => (
              <button
                ...
                style={{
                  padding: '10px 14px',
                  background: selectedMembers().includes(friend.id) ? m.gradients.primary : 'transparent',
...
export const FriendlyApp: Component = () => {
  return (
    <Show 
      when={store.state.viewMode === 'dashboard'} 
      fallback={<PublicView />}
    >
      <DashboardView />
    </Show>
