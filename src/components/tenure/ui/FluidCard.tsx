 ...
 *
 * Shared Tenure UI component for all Tenure modules (Pipeline, Prepare, Prosper, Discover).
 *
...
export const FluidCard: Component<FluidCardProps> = (props) => {
  ...
  const baseStyle = (): JSX.CSSProperties => {
    ...
    const variants = {
      ...
      elevated: {
        background: liquidTenure.colors.surface,
        border: `1px solid ${liquidTenure.colors.border}`,
        'box-shadow': liquidTenure.shadows.card,
      },
      outlined: {
    ...
    return {
      ...
      padding: liquidTenure.spacing.lg,
  ...
  const cardClass = () => {
    const classes = ['tenure-card', props.class || ''];
    if (props.variant === 'stat') classes.push('stat-card');
    if (props.hoverable) classes.push('tenure-card-hover');
